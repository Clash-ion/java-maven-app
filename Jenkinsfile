...
pipeline {
    ...
    stages {
        ...
        stage ('deploy') {
            ...
            steps {
                script {
                    env.ENV = input message: "Select the environment to deploy to", ok: "Done", parameters: [choice( name: 'ENV', description: 'Environment to deploy to', choices: ['dev', 'staging', 'prod'] )]
                    gv.deployApp()
